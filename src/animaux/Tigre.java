...
public class Tigre extends Charognard{
	 ...
	 * @param meurtFaim
	 */
	public Tigre(int dateNaissance,  Case emplacement,  int remplissageEstomac,  int maturite,
							...
							if (c.getAnimal().getAProcree() == false) {						// et enfin si l'autre animal n'a pas déjà procréé récemment 
								...
								for (Case cbis : cases){											// 			on cherche ensuite  
									if (c.getEstVide() == true) {									//			une case vide 
										Animal tigre = new Tigre (Gestionnaire.getTour(),cbis,  this.tailleEstomac/2,  this.getMaturite(),	// pour créer un nouvel individu
